...
- `data/venues_to_follow.json` — hand-maintained, local-only (2026-08-05): LA **venues** (not restaurants) worth following for a "dinner and a show" night (the Largo + Sushi Fumi pattern — walkable dinner pairing next to the venue), tagged by `category` — `intimate-listening-room`, `big-rock-club`, `disco-dance`, `outdoor` (Hollywood Bowl/Greek/Ford/Cinespia) — and by `area` (`westside`/`central`/`hollywood`/`east`/`dtla`), since Kevin/Hanna live in Brentwood and closer is preferred, especially for comedy/music (see the `location-venue-preferences` memory). `weekendSocialSpots` is a separate field — plain restaurant/bar hangout ideas grouped by neighborhood (Marina del Rey, Venice, Larchmont, since most friends are there); these were merged into `favorite_places_raw.json` as `want-to-go` entries on 2026-08-05, so `venues_to_follow.json` itself is only a research log for them now, not the live copy. The `venues` array (comedy clubs, music rooms, outdoor amphitheaters) is **not** merged anywhere yet — nothing today knows how to suggest "go see a show," only "go eat here"; that's a separate, not-yet-built feature (see below).
- **Not yet built** (explicitly scoped out of the 2026-08-05 ranking work, each a real feature of its own): a suggestion engine for `venues_to_follow.json`'s `venues` array (comedy/music/outdoor — needs real upcoming-show-date research this project hasn't done yet); an interactive "Dining + Shows" dashboard tab (star/rank UI that writes back into `favorite_places_raw.json`); the Telegram recap's redesign from free-form prose to structured Budget/Todos/Planning sections (discussed but not started).
