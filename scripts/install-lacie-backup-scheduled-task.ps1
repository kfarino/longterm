...
# Daily mirror of Documents\Longterm + nikola + ~/.longterm → Lacie\Projects.
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings `
    -Description 'Daily robocopy of Documents\Longterm + nikola + ~/.longterm to Lacie\Projects (includes gitignored finance data). Skips if Lacie unplugged. Log: %USERPROFILE%\.longterm\logs\lacie-backup.log' `
    -Force | Out-Host
