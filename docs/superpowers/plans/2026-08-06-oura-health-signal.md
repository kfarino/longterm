...
**Also fixes a pre-existing red test.** `get_dining_plan` calls `nextDateForDayOfWeek(slot.dayOfWeek)` with no `from`, so it reads the real system clock even when a caller injects `now`. `data/test-telegram-recap.mjs`'s "already covered by an evening calendar event" case pins `now: SUNDAY` (2026-08-02) and expects family_dinner to resolve to 2026-08-05 — it silently went red when the real date passed 08-05. Thread `now` through as part of this task (Step 3b below), since this task already edits that call site.
...
export function get_dining_plan(monthPlanEvents, { occasion, now = null }, diningContext, extraExcludeNames = new Set()) {
  ...
  // `now` defaults to the real clock; callers that pin a date (the recap, and
  // any test injecting a fixed `now`) need it to reach the date math, not just
  // the surrounding bundle.
  const date = nextDateForDayOfWeek(slot.dayOfWeek, now || new Date());
```

In `scripts/telegram-bot-recap.mjs`, pass the recap's own `now` through `diningSummary`:

```js
function diningSummary(monthPlanEvents, diningContext, now) {
  const summary = {};
...
Update `gatherBundle` to accept and forward `now` to its `diningSummary(monthPlanEvents, diningContext, now)` call.
