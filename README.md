...
Kevin & Hanna's financial planning system: a dashboard, a Telegram bot, and the automation that keeps them both in sync with real account/budget data.
...
These are the files every script in this project reads and writes at runtime — the bot, the dashboard, and the scheduled Monarch pulls all depend on them existing locally, but none of that is a git dependency. The system runs entirely off local files; git/GitHub here is just a backup of the code and tooling, not part of the running system. A fresh clone of this repo will need those 5 files created locally (by hand, or by copying them in) before the dashboard or bot will have anything real to show.
...
A handful of `.jsonl`/`.json` files track bot/automation state (Telegram offset, sender ownership, conversation history, recap dedup log, etc.). These aren't financial data — just bookkeeping for the bot's own operation — so they're committed for continuity/debugging, unlike the 5 files above.
